...
static ssize_t tmp100_read(struct file * filp, char *buffer_user, size_t length, loff_t *offset) {
	...
	printk(KERN_INFO "Fraction part: %d\n", ((unsigned short)r_val >> 4) & 0x0f);
	...
	// And the 4 LSB of these 12 bits are used for the fraction part, which means we can have 16 different fraction parts 
	// We multiply r_val by some number, depending on where we want our floating point 
	snprintf(output, BUFF_SIZE, "%d.%04d", (unsigned short)r_val >> 8, // Whole part
