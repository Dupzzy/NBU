...
static int tmp100_open(struct inode *inode, struct file *file) {
	// In case of something is trying to open the file for executing operations with it like
	// reading while it's open, the process will be terminated and the operation wont be executed
	// returning "operation is not permitted" error
	if (tmp100.is_open == 1) {
...
static ssize_t tmp100_read(struct file * filp, char *buffer_user, size_t length, loff_t *offset) {
	...
	// For example if we get -10 in r_val
	// Before r_val = -r_rval; 1111 0110 0000 0000
	// r_val = -r_val;
	// r_val = ~r_val + 1;
	// 0000 1001 1111 1111 + 1
	// After: 0000 1010 0000 0000 positive 10d
	...
	integer_part =  (unsigned short)r_val >> 8;
	...
	if(*offset > length) // Check in case of overflow
		return -ENFILE;
